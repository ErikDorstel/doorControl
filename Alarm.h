...
void alarmWorker() {
  ...
  if (timerRead(hwTimer)>=alarmTimer || alarmFlag==1) {
    alarmFlag=0; attempt++; if (attempt>32) { attempt=32; }
    if (attempt<16) { alarmTimer=timerRead(hwTimer)+2000000ULL; } else { alarmTimer=timerRead(hwTimer)+60000000ULL; }
    alarmUDP.begin(9999);
  ...
  if (alarmUDP.parsePacket()==6) {
    ...
    attempt=0; alarmTimer=timerRead(hwTimer)+60000000ULL; } }
